...
duration_pattern = re.compile(r'for the past \d+ (days|weeks|months|years)|since last \w+|for \w+')
...
all_data['Body Part'] = all_data['Vignette'].apply(extract_body_part)  # Ensure this is correct
all_data['Duration'] = all_data['Vignette'].apply(extract_duration)    # Ensure this is correct
...
def extract_sentiment(text):
    ...
    elif 0.05 <= sentiment_score < 0.6:
        return 'Positive'
    elif -0.05 < sentiment_score < 0.05:
        return 'Neutral'
    elif -0.6 <= sentiment_score < -0.05:
        return 'Negative'
    else:
        return 'Very Negative'

# Extend the emotion categories
emotion_keywords = {
    'joy': ['happy', 'joyful', 'excited', 'delighted', 'cheerful'],
    'sadness': ['sad', 'depressed', 'unhappy', 'sorrowful', 'down'],
    'anger': ['angry', 'frustrated', 'irritated', 'mad', 'enraged'],
    'fear': ['afraid', 'scared', 'terrified', 'fearful', 'anxious'],
    'surprise': ['surprised', 'amazed', 'astonished', 'shocked'],
    'disgust': ['disgusted', 'repulsed', 'offended', 'sickened']
}
    ...
    for emotion, keywords in emotion_keywords.items():
        for keyword in keywords:
            if keyword in text:
                ...
                break  # Stop checking once we find one keyword for the emotion

    if detected_emotions:
        return list(set(detected_emotions))  # Return unique emotions found
    else:
...
def process_user_input(user_input):
    ...
    emotions = extract_emotion(user_input)  # Emotion analysis based on original input
    ...
    return {
        ...
        'Duration': duration,
        'Sentiment': sentiment,
        'Emotions': emotions  # Include extracted emotions
    }
